...
def create_knowledge_base(create_data: PublicCreateDatasetRequest):
    ...
    return JSONResponse(content={"error": "Failed to create dataset", "status_code": response.status_code}, status_code=response.status_code)
...
def get_knowledge_base_list(page: int = 1, limit: int = 20):
    ...
    if response.status_code == 200:
        return response.json()  
    return JSONResponse(content={"error": "Failed to get knowledge base list", "status_code": response.status_code}, status_code=response.status_code)
...
def delete_knowledge_base(dataset_id: str):
    ...
    if response.status_code == 200:
        return response.json()  
    return JSONResponse(content={"error": "Failed to delete knowledge_base", "status_code": response.status_code}, status_code=response.status_code)
...
def create_document_by_text(dataset_id: str, create_data: CreateDocumentByText):
    ...
    if response.status_code == 200:
        return response.json()
    print(response.json())
    return JSONResponse(content={"error": "Failed to create document", "status_code": response.status_code}, status_code=response.status_code)

# Update a Document with Text ✅
@app.post("/datasets/{dataset_id}/documents/{document_id}/update-by-text")
def update_document_by_text(dataset_id: str, document_id: str, update_data: UpdateDocumentByText):
    ...
    if response.status_code == 200:
        ...
        print(response.json())
        return JSONResponse(
            content={"error": "Failed to update document", "status_code": response.status_code},
...
async def create_document_with_file (dataset_id: str, file: UploadFile = File(...)):
    ...
    files = {
        "file": (file.filename, await file.read(), 'text/plain')
    }
    try:
        response = requests.post(url, headers = dataset_headers_file, data = data, files = files)
        
        if response.status_code == 200:
            return response.json()  # Returning the API response back to the user
        else:
            raise HTTPException(status_code=response.status_code, detail="Error from Dify API")
...
async def update_document_with_file(dataset_id: str, document_id: str, file: UploadFile = File(...)):
...
def delete_document(dataset_id: str, document_id: str):
    ...
    if response.status_code == 200:
        return response.json()
    return JSONResponse(content={"error": "Failed to delete document", "status_code": response.status_code}, status_code=response.status_code)
...
def get_document_list(dataset_id: str):
    ...
    if response.status_code == 200:
        return response.json()  
    return JSONResponse(content={"error": "Failed to get document list", "status_code": response.status_code}, status_code=response.status_code)
...
def send_chat_message(chat: ChatMessage):
    ...
    if response.status_code == 200:
        ...
        return {"error": "Failed to send the message", "status_code": response.status_code, "message": response.text}
...
def get_conversations():
    ...
    if response.status_code == 200:
        ...
        return {"error": "Failed to fetch conversations", "status_code": response.status_code}
